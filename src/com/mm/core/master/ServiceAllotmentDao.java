...
public class ServiceAllotmentDao {
	...
	public Vector getServiceAllotmentList(DataSource  datasource, int facilityId) throws Exception {
			...
			try {
				...
				String sql = DBConstant.SELECT_SERVICEFOR_ALLOTMENT_SQL;
				//select A.mappingid,A.serviceid,CONCAT_WS('  ',B.first_name,B.last_name) AS NAME,C.tower,C.flat,
				//D.servicename,A.status from consumerservicemapping A , loginmaster B,resident_details C,service D Where A.consumerid=B.userid AND A.consumerid=C.user_id AND A.serviceid=D.serviceid  AND A.status=1
				...
				while (rs.next()) {
					ServiceAllotmentBean bean = new ServiceAllotmentBean();
					bean.setMappingId(rs.getInt("mappingid"));
					bean.setServiceId(rs.getInt("serviceId"));
					bean.setConsumerName(rs.getString("NAME"));
					bean.setTowerName(rs.getString("tower"));
					bean.setFlatNo(rs.getString("flat"));
		...
		public ServiceAllotmentBean getServiceAllotmentData(DataSource  datasource, int serviceId,int mappingId) throws Exception {
			Connection con = null;
			PreparedStatement pStatement = null;
			ResultSet rs = null;
			ServiceAllotmentBean bean = new ServiceAllotmentBean();
			try {
				con = datasource.getConnection();
				String sql = DBConstant.SELECT_SERVICE_ALLOTMENT_SQL;
				pStatement = con.prepareStatement(sql);
				System.out.println("sqlsqlsql Allotment Dao->"+sql);
				
				pStatement.setInt(1, mappingId);
				pStatement.setInt(2, serviceId);
				
				System.out.println("mappingId->"+mappingId);
				System.out.println("serviceId->"+serviceId);
				//select A.mappingid,A.serviceid,CONCAT_WS('  ',B.first_name,B.last_name) AS NAME,C.tower,C.flat,D.servicename,A.status from consumerservicemapping A , loginmaster B,resident_details C,service D Where A.consumerid=B.userid AND A.consumerid=C.user_id AND A.serviceid=D.serviceid  AND A.status=1 AND A.mappingid=177 AND A.serviceid=1;
				rs = pStatement.executeQuery();
				if (rs.next()) {
					bean.setMappingId(rs.getInt("mappingid"));
					bean.setServiceId(rs.getInt("serviceId"));
		...
		public void insertService(DataSource  datasource, ServiceAllotmentForm serviceBean) throws Exception {
			Connection con = null;
			PreparedStatement pStatement= null;
...
//					+ "createdby,createdon,modifiedby) " + "values (?,?,?,?,?,?,?)";
		...
		public void updateService(DataSource  datasource, ServiceAllotmentForm serviceBean) throws Exception {
			Connection con = null;
			PreparedStatement pStatement = null;
			try {
				con = datasource.getConnection();
				String sql = DBConstant.UPDATE_CONSUMER_SERVICE_MAPPING_ALLOTMENT_SQL;
				System.out.println("serviceBean.getStatus()-->"+serviceBean.getStatus());
				System.out.println("serviceBean.getModifiedBy()"+serviceBean.getModifiedBy());
				System.out.println("sserviceBean.getMappingId()-->"+serviceBean.getMappingId());
				System.out.println("serviceBean.getConsumerId()-->"+serviceBean.getConsumerId());
				System.out.println("serviceBean.getRemarks()-->"+serviceBean.getRemarks());
//update consumerservicemapping set status=?,modifiedby=?,modifiedon=CURDATE() where mappingid=? AND consumerId=?
				
				pStatement = con.prepareStatement(sql);

				pStatement.setInt(1,serviceBean.getStatus());
				pStatement.setInt(2, serviceBean.getModifiedBy());
				pStatement.setString(3, serviceBean.getRemarks());
				pStatement.setInt(4, serviceBean.getMappingId());
				pStatement.setInt(5, serviceBean.getConsumerId());
		...
		public void insertConsumerBillDetail(Vector dataList, Connection con)
			...
			try {
				...
				for (int i = 0; i < dataList.size(); i++) {
					ServiceAllotmentBean serviceBean = (ServiceAllotmentBean) dataList.get(i);
					pStatement.setInt(1, serviceBean.getConsumerId());
					pStatement.setInt(2, serviceBean.getServiceId());
...
*/
		...
		public void processMails(Vector mailMessageDetails) {
			...
			for (int i = 0; i < size; i++) {
				...
				for (int j = 0; j < innerSize; j++) {
					ServiceAllotmentBean bean = (ServiceAllotmentBean) mailMessageDetails.get(i);
		...
		public String getMailMsg(ServiceAllotmentBean bean) {
