...
public class CountrystatecityAction extends Action {
	...
	private String getTableRow(String strCountries, String strState, String strPlace, int numTempCount)
	{
		...
		{
			strResult+="<td  width=\"25%\" class=\"bold\" style=\"cursor:hand\" onclick=\"retrieveCountryStateCity('countrystatecity.do?place=0&state=0&country=94~+91');\"><b><u><font color=\"blue\">Switch Back To List Mode</font></u></b></td>";
		}		
		strResult+="</tr>";
		strResult+="<tr height=\"20\">";
		strResult+="<td>&nbsp;</td>";
		...
		{
			strResult+="<td  class=\"bold\" style=\"cursor:hand\" onclick=\"retrieveCountryStateCity('countrystatecity.do?place=0&state=0&country=94~+91');\"><b><u><font color=\"blue\">Switch Back To List Mode</font></u></b></td>";
		}
		strResult+="</tr>";
		strResult+="<tr height=\"20\">";
		strResult+="<td>&nbsp;</td>";
	...
	private String getCountriesList(int numCid, ArrayList CountryList, int numSId, int numPId, int tabindex, String strCIdOth)
	{
		...
		{
			strResult ="<select name=\"country\"  tabindex=\""+tabindex+"\" onchange=\"retrieveCountryStateCity('countrystatecity.do?place="+numPId+"&state="+numSId+"&country='+this.value), setCountry()\">";
	...
	private String getStateList(int numSid, int numCId, HashMap stateList, int numPId, int tabindex,String strSIdOth)
	{
		...
		if(AllStates.size()>2)
		{
			strResult ="<select name=\"state\"  tabindex=\""+tabindex+"\" onchange=\"retrieveCountryStateCity('countrystatecity.do?country="+numCId+"&place="+numPId+"&state='+this.value);\">";
	...
	private String getPlaceList(int numPId, HashMap citiesList, int numSId, int numCId, int tabindex, String strPIdOth)
	{
		...
		if(AllCities.size()>2)
		{
			strResult ="<select name=\"city\"   tabindex=\""+tabindex+"\"  onchange=\"retrieveCountryStateCity('countrystatecity.do?country="+numCId+"&state="+numSId+"&place='+this.value);\">";
