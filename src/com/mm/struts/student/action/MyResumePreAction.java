...
public class MyResumePreAction extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(qtype1.equalsIgnoreCase("detail"))
		{
			...
			////Systemout.println("respDataVec>>>"+respDataVec);
			 responseVec = rootMaster.getResponseList(getDataSource(request, "student"), respDataVec);
		...
		int numCompId  = indvMaster.getCompanyId(getDataSource(request, "student"), numUId);		
		...
		{
			 userMTlist = indvMaster.getMTList(getDataSource(request, "student"), paramVec);
		...
		{
			 userVeclist = indvMaster.getComplaintList1(getDataSource(request, "student"), paramVec);
			 System.out.println("userVeclist...userVeclist.....getComplaintList1........" +userVeclist);
		}
		...
		Vector userVec = indvMaster.getUserInfo(getDataSource(request, "student") ,Integer.parseInt(uId));
		...
		String contryStr = rootMaster.getCountryName(getDataSource(request, "student"),contryId);
		int stateId = Integer.parseInt(userVec.elementAt(7).toString());//dealer stateId id
		String stateStr = rootMaster.getStateName(getDataSource(request, "student"),stateId);
		int cityId = Integer.parseInt(userVec.elementAt(6).toString());//dealer cityId id
		String cityStr = rootMaster.getPlaceName(getDataSource(request, "student"),cityId);
