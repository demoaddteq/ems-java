...
import com.mm.struts.student.form.SearchComplaintForm;
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,HttpServletRequest request, HttpServletResponse response) {
		SearchComplaintForm SearchComplaintForm = (SearchComplaintForm) form;// TODO Auto-generated method stub
		 ...
		 HttpSession session = request.getSession();
			...
			if(SearchComplaintForm.getTag().length()>0)
			{
				datavec.add("complaint_title like");
				datavec.add("%"+SearchComplaintForm.getTag()+"%");
			}
			
			if(SearchComplaintForm.getCompText().length()>0)
			{
				datavec.add("complaint_text like");
				datavec.add("%"+SearchComplaintForm.getCompText()+"%");
			}
