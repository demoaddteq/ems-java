...
public class StudentResumePreAction extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		String uId = (request.getParameter("userid")!=null) ? request.getParameter("userid") : (String)session.getAttribute("uId");
		...
		if(qtype1.equalsIgnoreCase("detail"))
		{
			...
			respDataVec.add("student");
			////Systemout.println("respDataVec>>>"+respDataVec);
			 responseVec = rootMaster.getResponseList(getDataSource(request, "main"), respDataVec);
		...
		int numCompId  = indvMaster.getCompanyId(getDataSource(request, "main"), numUId);		
		...
		Vector countVec = indvMaster.complaintCount(getDataSource(request, "main"), dataVec);		
		...
		int totalRow = indvMaster.getComplaintCount1(getDataSource(request, "main"), paramVec1);
		...
		Vector userVeclist=new Vector();
		 ...
		 Vector expVec=indvMaster.getExpDetails( getDataSource(request, "main") ,Integer.parseInt(uId));
		...
		{
			 userMTlist = indvMaster.getMTList(getDataSource(request, "main"), paramVec);
		...
		{
			 userVeclist = indvMaster.getComplaintList1(getDataSource(request, "main"), paramVec);
		...
		Vector userVec = indvMaster.getUserInfo(getDataSource(request, "main") ,Integer.parseInt(uId));
		...
		String contryStr = rootMaster.getCountryName(getDataSource(request, "main"),contryId);
		int stateId = Integer.parseInt(userVec.elementAt(7).toString());//dealer stateId id
		String stateStr = rootMaster.getStateName(getDataSource(request, "main"),stateId);
		int cityId = Integer.parseInt(userVec.elementAt(6).toString());//dealer cityId id
		String cityStr = rootMaster.getPlaceName(getDataSource(request, "main"),cityId);
		...
		Vector resultVec = entpMaster.getCommunityUnnamedMap( getDataSource(request, "main") ,numCompId);	
		...
		consumerUnName = rootMaster.getUserUnNameInfo(getDataSource(request, "main"), numUId, numCompId );
