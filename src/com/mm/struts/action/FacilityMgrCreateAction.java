...
import com.mm.struts.form.FacilityMgrPreForm;

public class FacilityMgrCreateAction extends Action {	
	
	
public ActionForward execute(ActionMapping mapping, ActionForm form,HttpServletRequest request, HttpServletResponse response) throws Exception {
	HttpSession session =request.getSession();	
	session.setAttribute("flagforSr", "1");
	//System.out.println("Ajay111111111111111111");
	if(((String)session.getAttribute("flagforSr")).equalsIgnoreCase("1"))
	{
		
	
		FacilityMgrPreForm communitySUnnameForm = (FacilityMgrPreForm) form;
	...
	String companyType="Corporates";
	Vector CompIdVec =entpMaster.getCompanyId(ds, strComName);
 	
	Vector BuildVec = (Vector)request.getAttribute("datavec");
	//System.out.println("BuildVec is "+BuildVec);
	
	//String Compid= BuildVec.elementAt(0).toString();
...
//	System.out.println("resultVec//////CommunitySUnnameAction////////"+resultVec);
	...
	*/
	...
	//	result1 = rootMaster.insertFMInfo(dataSource,infoVec,userId);
...
Vector <Vector>educationVecFinal = new Vector<Vector>();
	...
	for(int j=0; j <educationVecFinal.size(); j++ ){
		...
		String result1 = rootMaster.insertEducationInfo(dataSource, teampVec, String.valueOf(userId));
