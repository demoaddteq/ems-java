...
public class ComplaintsDetailAction  extends Action
{
...
//	      get all company list with the no of responses accorrding to company type. rootmaster.getResponseList()
			...
			Vector responseVec = rootMaster.getResponseList(getDataSource(request, "main"), respDataVec);
	...
	{   
		...
		strvalue+="<div style=\"float:left; width:98%\">";
		...
		if(detailVec.size()!=0){
			...
			//	for check length of Complaint title
				 ...
				 String strFMsg1 = "";
					...
					strvalue+="<div class=\"classified_DetailsLFT\">Comments:</div><div class=\"classified_DetailsRHT\">"+infoVec.elementAt(3).toString().trim()+"</div>";
					strvalue+="</div>";
		...
		if(typ.equalsIgnoreCase("blog"))
		{
			/*		if(responseVec.size()>0)
					{	
						...
						for(int i=0;i<responseVec.size();i++)
						{
							...
							strvalue += "<div class=\"commentfooter\">Posted by &nbsp;";
								if(comptype.equalsIgnoreCase("Facility Manager")){
					...
					else
					{
						...
						strvalue+="<td colspan=\"4\"  class=\"required\"> No Response</td>";
						strvalue+="</tr>";
					}
				*/	
		...
		strvalue+="<tr>";
		strvalue+="<td height=\"150\" align=\"center\"  class=\"required\"><h3>Clasified does not exist for given reference.</h3> </td>";
		strvalue+="</tr>";
		strvalue+="</table>";
		}
		strvalue+="<br></td>";
		...
		if(Stype.equalsIgnoreCase("first")||(Stype.length()==0))
		{
			...
			{
				strvalue+="<a href=\"#\" style=\"cursor:hand\" onclick=\"javascript:back()\">Back</a>";
				strvalue+="<a href=\"#\">Back to Blog(s) List</a>";
			...
			strvalue+="<div style=\"float:left; width:95%; border-top:2px solid #ccc; margin:10px 0px;\">";
			strvalue+="<p style=\"text-align:center; padding:10px; font-size:11px; font-weight:bold;\">Note : You should be a registered user(i.e student, college or company) to post your reply.</p>";
			strvalue+="<div class=\"inputfield\" style=\"margin-top:10px;\">";
			strvalue+="<label for=\"loginId\">User Name:</label> <input type=\"text\" id=\"loginId\" name=\"loginId\"/>";
			strvalue+="<label for=\"pasword\">Password:</label> <input type=\"password\" id=\"pasword\" name=\"pasword\"/>";
			strvalue+="<input type=\"hidden\" value=\"indv\" name=\"type\"/>";
			strvalue+="</div>";
			
	                        strvalue+="<TD width=\"60%\"><input type=\"hidden\" name=\"complaintId\" value=\""+complainid+"\"></html:hidden></TD>";
		...
		{	
			...
			strvalue+="<td ><input type=\"hidden\" name=\"complaintId\" value=\""+complainid+"\"></html:hidden>" +
					"<input type=\"hidden\" name=\"userId\" value=\""+strUserId+"\"></html:hidden><table><tr><td valign=\"top\" align=\"left\" width=\"18%\"><span ><strong>Comment Text:</strong></span></td><td valign=\"top\" align=\"left\"><span ><textarea  name=\"comtext\" cols=\"70\" rows=\"5\"  ></textarea></span></td></td>";
			strvalue+="</tr>";
			strvalue+="</table>";
			strvalue+="</td>";
			strvalue+="</tr>";
