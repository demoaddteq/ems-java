...
public class LoginAction extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		else
		{
			...
			System.out.println("user Side Vec"+ userSideVec);
			System.out.println("country is +");
				String contryStr1 = rootMaster.getCountryName(dataSource,94);
	    		String stateStr1 = rootMaster.getStateName(dataSource,1);	
	    		String cityStr1 = rootMaster.getPlaceName(dataSource,211);
    	...
    	//	String cityStr1 = rootMaster.getPlaceName(dataSource,Integer.parseInt(userSideVec.elementAt(6).toString()));
			...
			else
			{
				...
				if(type.equalsIgnoreCase("indv"))
				{
					...
					Vector countFVec = rootMaster.getfacility(getDataSource(request, "main"), strUserId);
					session.setAttribute("fId", countFVec.elementAt(0).toString().trim());
					session.setAttribute("fName", countFVec.elementAt(2).toString().trim());	
				    System.out.println("facility name is  "+countFVec);
				    
				    Vector FIDVec = rootMaster.getResfacID(getDataSource(request, "main"), strUserId);	
				    String cmid = "NA";
				    String FUNAME = "NA";
				    String FULNAME = "NA";
				    
				    String check_facility="1";
				    String fUser = "1";
					if (FIDVec.size()!=0){
						fUser =  FIDVec.elementAt(0).toString().trim();
				   
					
					String fu_id=FIDVec.elementAt(0).toString().trim();
					Vector FacilityVec = rootMaster.getfacilitydetails(getDataSource(request, "main"), fu_id);
				System.out.println("facility user name is  "+FacilityVec);
				cmid = FacilityVec.elementAt(0).toString().trim();
				FUNAME = FacilityVec.elementAt(1).toString().trim();
				FULNAME =  FacilityVec.elementAt(2).toString().trim();
					}
					else
						{check_facility="0";} session.setAttribute("fuId",fUser);
						System.out.println("facility ID is  "+fUser);
					session.setAttribute("FM_exist_ID", check_facility);					
					
					System.out.println("Exists Facility "+check_facility);
					Vector <String> FDetailsVec = new Vector<String>();	
					FDetailsVec.add(cmid);
					FDetailsVec.add(FUNAME);
					FDetailsVec.add(FULNAME);
			...
			//		System.out.println("facility user name is  "+FacilityVec);
					...
					session.setAttribute("BuilderVec",FDetailsVec);
				...
				else if(type.equalsIgnoreCase("corpo"))
				{
					...
					Vector countFIDVec = rootMaster.getfacID(getDataSource(request, "main"), strUserId);		
					session.setAttribute("fId", countFIDVec.elementAt(0).toString().trim());
					session.setAttribute("UserId", strUserId);
					Vector countFVec = rootMaster.getfacilityinfo(getDataSource(request, "main"), strUserId);
					session.setAttribute("bId", countFVec.elementAt(0).toString().trim());
					session.setAttribute("fuId", countFVec.elementAt(1).toString().trim());	
					session.setAttribute("fName", countFVec.elementAt(2).toString().trim());	
					System.out.println("facility name is  "+countFVec);
					
					Vector FacNameVec = rootMaster.getfacilitydetails(getDataSource(request, "main"),strUserId);
					//	System.out.println("facility user name is  "+FacilityVec);
						session.setAttribute("cmId", FacNameVec.elementAt(0).toString().trim());					
						session.setAttribute("fUName", FacNameVec.elementAt(1).toString().trim());
						session.setAttribute("fULName", FacNameVec.elementAt(2).toString().trim());
				
					Vector<String> dataVec2 = new Vector<String>();		
					dataVec2.add(strUserId);
					dataVec2.add(userData.elementAt(0).toString().trim());	
					dataVec2.add(userRightData.elementAt(18).toString().trim());	
					Vector servicesVec = advtMaster.getServices(getDataSource(request, "main"), Integer.parseInt(countFVec.elementAt(1).toString().trim())); 
					session.setAttribute("ServiceList", servicesVec);
					System.out.println("User right data value"+userRightData.elementAt(18).toString().trim());
			//		Vector advtCountVec = advtMaster.complaintCountAdvt(getDataSource(request, "main"), dataVec2);		
			//		session.setAttribute("advtCountVec", advtCountVec);						
