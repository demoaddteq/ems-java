...
public class ResidentsServicesAction extends Action{
	public ActionForward execute(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response) throws Exception 
	{
		System.out.println("In ResidentsServicesAction Action");
		RootMaster rootMaster=new RootMaster();
		
		String category_Id = (request.getParameter("categoryId")!=null) ? (request.getParameter("categoryId").trim()) : "0";
		String suCategory_Id = (request.getParameter("suCategoryId")!=null) ? (request.getParameter("suCategoryId").trim()) : "0";

		int catId=Integer.parseInt(category_Id);
		int subCatId=Integer.parseInt(suCategory_Id);
		
		Vector streamVec = rootMaster.getResidentsServices(getDataSource(request,"corpo"),1);
	...
	private String getCourseStreamList(int numSid, int catId, Vector streamVec)
	{
		...
		String strResult ="<select name=\"serviceId\">";
