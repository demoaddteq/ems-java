...
public class BuilderAction extends Action{
	...
	{
		System.out.println("In Builder Action");
		...
		Vector streamVec = rootMaster.getBuilderList(getDataSource(request, "corpo"), 1);
	...
	private String getCourseStreamList(int numSid, int numCId, Vector streamVec)
	{
		...
		String strResult ="<select name=\"builder\" onchange=\"retrieveBuilderCityURL('setbuildercity.do?builderId='+this.value)\">";
		System.out.println("strResult Builder Passed For Retrieve City--03092009=>"+strResult);
		...
		strResult += "</select>";
		System.out.println("strResult 07092009-1-->"+strResult);
		return strResult;
	}
