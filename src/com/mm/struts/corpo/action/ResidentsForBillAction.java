...
public class ResidentsForBillAction extends Action{
	public ActionForward execute(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response) throws Exception 
	{
		System.out.println("In ResidentsForBillAction Action");
		RootMaster rootMaster=new RootMaster();
		
		HttpSession session=request.getSession();
		int facilityId = (session.getAttribute("fId")!=null) ? Integer.parseInt((String)session.getAttribute("fId")) : 0;
		
		Vector streamVec = rootMaster.getResidentsForBillGeneration(getDataSource(request,"corpo"),facilityId);
	...
	private String getCourseStreamList(int numSid, int catId, Vector streamVec)
	{
		...
		String strResult ="<select name=\"residentId\">";
