...
import com.mm.core.master.StaffLeaveDao;
 ...
 */
public class StaffLeaveAction extends Action {
	/*
	 * Generated Methods
	 */
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		StaffLeaveForm staffLeaveForm = (StaffLeaveForm) form;
		StaffLeaveDao staffLeaveDao = new StaffLeaveDao();
		...
		try {
			if (operation.equals("addnew")) {
				String results = null;//validateServiceForm(staffLeaveForm);
				if (results != null) {
					request.setAttribute(APPConstant.MESSAGE, results);
					forward = "addStaffFailure";
				}
				else {
					staffLeaveDao.insertDetail(dataSource, staffLeaveForm);
					Vector consumerList = staffLeaveDao.getLeaveList(dataSource, facilityId);
					request.setAttribute(APPConstant.STAFFLEAVE_LIST, consumerList);
					request.setAttribute(APPConstant.MESSAGE, APPConstant.INSERT_SUCCESS);
					forward = "showLeave";
				}
			} else if (operation.equals("edit")) {
				staffLeaveDao.updateLeaveRequest(dataSource, staffLeaveForm);
				Vector consumerList = staffLeaveDao.getLeaveList(dataSource, facilityId);
				request.setAttribute(APPConstant.STAFFLEAVE_LIST, consumerList);
				...
				deleteLeaveRequest(request, facilityId);
			} else if (operation.equals("showLeave")) {
				Vector leaveList = staffLeaveDao.getLeaveList(dataSource, facilityId);
				request.setAttribute(APPConstant.STAFFLEAVE_LIST, leaveList);
			}else if (operation.equals("searchService")){
				String serviceName=request.getParameter("serviceName"); 
				Vector consumerList = staffLeaveDao.searchServices(dataSource, serviceName);
				request.setAttribute(APPConstant.STAFFLEAVE_LIST, consumerList);
			}else if (operation.equals("showServiceResident")){
				int consumerId = getConsumerId(session);
				//int facilityId = getFacilityId(session);
				String serviceName=request.getParameter("serviceName"); 
				Vector consumerList = staffLeaveDao.getServiceListResident(dataSource, consumerId, facilityId);
				request.setAttribute(APPConstant.STAFFLEAVE_LIST, consumerList);
	...
	private void deleteLeaveRequest(HttpServletRequest request, int facilityId)
			throws Exception {
		DataSource dataSource = getDataSource(request, "corpo");
		String leaveId = request.getParameter("leaveId");
		String leaveIdsArr[] = leaveId.split(",");
		
		StaffLeaveDao staffLeaveDao = new StaffLeaveDao();
		staffLeaveDao.inactiveLeaveRequest(dataSource, leaveIdsArr);
		Vector LeaveList = staffLeaveDao.getLeaveList(dataSource, facilityId);
		request.setAttribute(APPConstant.STAFFLEAVE_LIST, LeaveList);
	...
	private void setDataForEditPage(HttpServletRequest request)
		...
		StaffLeaveBean bean = staffLeaveDao.getLeaveData(dataSource, leaveId);
		request.setAttribute(APPConstant.LEAVEDATAUPDATE, bean);
	}
	
	private String validateServiceForm(StaffLeaveForm staffLeaveForm ) {
		
		if (staffLeaveForm == null) {
			return "Please fill New Staff Leave form";
		}
		return null;
