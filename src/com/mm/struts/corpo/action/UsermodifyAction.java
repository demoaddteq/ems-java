		...
		if(actionTask.equalsIgnoreCase("Delete")){
		 ...
		 String tempCountry[] = usermodifyForm.getCountry().split("~");
			...
			if(result.equalsIgnoreCase("success"))
			{
				...
				int numStatus = am.getUserCatStatus(getDataSource(request, "corpo"), Integer.parseInt(strCompId.trim()), Integer.parseInt(strUId.trim()));
				if(numStatus>0)
				{
