...
import com.mm.core.master.RootMaster;
	...
	if(qtype1.equalsIgnoreCase("detail"))
	{
		...
		respDataVec.add("corpo");
		////Systemout.println("respDataVec>>>"+respDataVec);
		 responseVec = rootMaster.getResponseList(getDataSource(request, "corpo"), respDataVec);
	...
	int numCompId  = indvMaster.getCompanyId(getDataSource(request, "corpo"), numUId);		
	...
	Vector countVec = indvMaster.complaintCount(getDataSource(request, "corpo"), dataVec);		
	...
	int totalRow = indvMaster.getComplaintCount1(getDataSource(request, "corpo"), paramVec1);
	...
	Vector userVeclist=new Vector();
	 ...
	 Vector expVec=indvMaster.getExpDetails( getDataSource(request, "corpo") ,Integer.parseInt(uId));
	...
	{
		 userMTlist = indvMaster.getMTList(getDataSource(request, "corpo"), paramVec);
	...
	{
		 userVeclist = indvMaster.getComplaintList1(getDataSource(request, "corpo"), paramVec);
	...
	Vector userVec = indvMaster.getUserInfo(getDataSource(request, "corpo") ,Integer.parseInt(uId));
	...
	String contryStr = rootMaster.getCountryName(getDataSource(request, "corpo"),contryId);
	int stateId = Integer.parseInt(userVec.elementAt(7).toString());//dealer stateId id
	String stateStr = rootMaster.getStateName(getDataSource(request, "corpo"),stateId);
	int cityId = Integer.parseInt(userVec.elementAt(6).toString());//dealer cityId id
	String cityStr = rootMaster.getPlaceName(getDataSource(request, "corpo"),cityId);
	
	String CId =userVec.elementAt(22).toString();
	////System.out.println("CId...CId.. in resume...uId........" +CId);
	...
	if(compnyid==0)
	{
		 ...
		 ////System.out.println("CId...CId.. in compnyidin action...compnyid........" +compnyid);
		 Cname=name.elementAt(0).toString().trim();
		 ////System.out.println("CId..Name. in compnyidin action...compnyid........" +name);
	...
	consumerUnName = rootMaster.getUserUnNameInfo(getDataSource(request, "corpo"), numUId, numCompId );
