...
//	validatin for the Usermodify form. it check other category,brand and contact person.
	...
	{
		errors.add("corpoUsermodifyForm",new ActionError("errors.resource.other","country"));
		if(this.otherstate.length()<=0)
		{
			errors.add("corpoUsermodifyForm",new ActionError("errors.resource.other","state"));
			if(this.othercity.length()<=0)
			{
				errors.add("corpoUsermodifyForm",new ActionError("errors.resource.other","city"));
			}
		}
	}
	else
