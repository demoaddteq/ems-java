 ...
 */
public class QueryResponseForm extends ActionForm {
	/*
	 * Generated fields
	 */
	...
	public ActionErrors validate(ActionMapping mapping, HttpServletRequest req)
	{
		...
		if(this.responsetext.length()<=0)
		{
			...
			errors.add("corpoResponseForm",new ActionError("errors.required","Response"));
			session.setAttribute("result", "failure");
		}
		else if(this.responsetext.length()>1500)
		{
			//System.out.println("dc  in form else if");
			errors.add("corpoResponseForm",new ActionError("errors.resource.length","Response","1500"));
			session.setAttribute("result", "failure");
		}
		else
		{
